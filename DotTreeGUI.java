...
public class DotTreeGUI extends DrawingGUI {
	...
	public void handleMousePress(int x, int y) {
		if (mode == 'a') {
			...
			// TODO: YOUR CODE HERE
			Dot dot = new Dot(x, y); // initialize dot
			tree.insert(dot); //insert the dot
		}
		else if (mode == 'q') {
	...
	public void drawTree(Graphics g, PointQuadtree<Dot> tree, int level) {
		...
		// TODO: YOUR CODE HERE
		int dot_x = (int) tree.getPoint().getX();
		int dot_y = (int) tree.getPoint().getY();
		g.fillOval(dot_x - dotRadius,dot_y - dotRadius,2* dotRadius,2*dotRadius); //draw dot
		g.drawLine(tree.getX1(), dot_y, tree.getX2(), dot_y); //draw horizontal line through dot
		g.drawLine(dot_x, tree.getY1(), dot_x, tree.getY2());//draw vertical line through dot


		// Recurse with children
		// TODO: YOUR CODE HERE
		level += 1; //bookkeeping for the levels

		//if a child exists in any quadrant, recurse in that quadrant
		if (tree.hasChild(1)){
			drawTree(g, tree.getChild(1), level);
		}
		if (tree.hasChild(2)){
			drawTree(g, tree.getChild(2), level);
		}
		if (tree.hasChild(3)){
			drawTree(g, tree.getChild(3), level);
		}
		if (tree.hasChild(4)){
			drawTree(g, tree.getChild(4), level);
		}
	}
